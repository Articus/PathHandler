...
TCHAR;
	...
	public function it_parses_media_type_without_parameters()
	{
		$this->beConstructedWith(self::TCHAR . '/' . self::TCHAR);
		$this->getType()->shouldBe(self::TCHAR);
		$this->getSubtype()->shouldBe(self::TCHAR);
	...
	public function it_parses_media_type_with_unquoted_parameter()
	{
		$this->beConstructedWith(self::TCHAR . '/' . self::TCHAR . self::SP . ';' . self::SP . self::TCHAR . '=' . self::TCHAR);
		$this->getType()->shouldBe(self::TCHAR);
		$this->getSubtype()->shouldBe(self::TCHAR);
		$this->getParameters()->shouldBe([[self::TCHAR, self::TCHAR]]);
	}
	...
	public function it_parses_media_type_with_empty_quoted_parameter()
	{
		$this->beConstructedWith(self::TCHAR . '/' . self::TCHAR . self::SP . ';' . self::SP . self::TCHAR . '=""');
		$this->getType()->shouldBe(self::TCHAR);
		$this->getSubtype()->shouldBe(self::TCHAR);
		$this->getParameters()->shouldBe([[self::TCHAR, '']]);
	}
	...
	public function it_parses_media_type_with_quoted_parameter()
	{
		$header = self::TCHAR . '/' . self::TCHAR . self::SP . ';' . self::SP . self::TCHAR . '="'. self::QDTEXT .'\\\\\\""';
		$this->beConstructedWith($header);
		$this->getType()->shouldBe(self::TCHAR);
		$this->getSubtype()->shouldBe(self::TCHAR);
		$this->getParameters()->shouldBe([[self::TCHAR, self::QDTEXT . '\\"']]);
	}
	...
	public function it_parses_media_type_with_several_parameters()
	{
		$this->beConstructedWith('a/b;c=d;e="f";g=h');
		$this->getType()->shouldBe('a');
		$this->getSubtype()->shouldBe('b');
		$this->getParameters()->shouldBe([['c', 'd'], ['e', 'f'], ['g', 'h']]);
	}

	public function it_parses_media_type_with_several_parameters_that_have_same_name()
	{
		$this->beConstructedWith('a/b  ;  c="d"  ;  c=e  ;  c="f"');
		$this->getType()->shouldBe('a');
		$this->getSubtype()->shouldBe('b');
		$this->getParameters()->shouldBe([['c', 'd'], ['c', 'e'], ['c', 'f']]);
	}

	public function it_matches_against_any_type_any_subtype_media_range()
	{
		$this->beConstructedWith('abc/def; ghi=jkl');
		$this->match('*/*')->shouldBe(true);
		$this->match('*/*; test=123')->shouldBe(true);
		$this->match('*/* ; test=123')->shouldBe(true);
		$this->match("*/*\t; test=123")->shouldBe(true);
	}

	public function it_matches_against_fixed_type_any_subtype_media_range()
	{
		$this->beConstructedWith('abc/def; ghi=jkl');
		$this->match('abc/*')->shouldBe(true);
		$this->match('abc/*; test=123')->shouldBe(true);
		$this->match('abc/* ; test=123')->shouldBe(true);
		$this->match("abc/*\t; test=123")->shouldBe(true);
		$this->match('xyz/*')->shouldBe(false);
		$this->match('xyz/*; test=123')->shouldBe(false);
		$this->match('xyz/* ; test=123')->shouldBe(false);
		$this->match("xyz/*\t; test=123")->shouldBe(false);
	}

	public function it_matches_against_fixed_type_fixed_subtype_media_range()
	{
		$this->beConstructedWith('abc/def; ghi=jkl');
		$this->match('abc/def')->shouldBe(true);
		$this->match('abc/def; test=123')->shouldBe(true);
		$this->match('abc/def ; test=123')->shouldBe(true);
		$this->match("abc/def\t; test=123")->shouldBe(true);
		$this->match('abc/xyz')->shouldBe(false);
		$this->match('abc/xyz; test=123')->shouldBe(false);
		$this->match('abc/xyz ; test=123')->shouldBe(false);
		$this->match("abc/xyz\t; test=123")->shouldBe(false);
		$this->match('xyz/def')->shouldBe(false);
		$this->match('xyz/def; test=123')->shouldBe(false);
		$this->match('xyz/def ; test=123')->shouldBe(false);
		$this->match("xyz/def\t; test=123")->shouldBe(false);
		$this->match('xyz/uvw')->shouldBe(false);
		$this->match('xyz/uvw; test=123')->shouldBe(false);
		$this->match('xyz/uvw ; test=123')->shouldBe(false);
		$this->match("xyz/uvw\t; test=123")->shouldBe(false);
	}
}
